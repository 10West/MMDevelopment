// Modified to rename "require" as "honeyRequire" to work along side require.js
...
//      afterwards!
    ...
    var requirePath = self.Honey&&self.Honey.requirePath!==undefined ? self.Honey.requirePath.slice(0) : ['./resources/'];
...
//      module has been loaded.
    ...
    function honeyRequire(identifier, callback, compiler) {
        if (identifier instanceof Array) {
            var modules = new Array();
            var modcount = identifier.length;
            for (var index = 0; index < identifier.length; index++) {
                (function(id, i) {
                    modules.push(honeyRequire(id, callback&&function(mod) {
                        modules[i] = mod;
...
// INFO Exporting require to global scope
    ...
    if (self.honeyRequire !== undefined)
        throw new Error("Honey: '\'require\' already defined in global scope");

    try {
        Object.defineProperty(self, 'honeyRequire', {'value':honeyRequire});
        Object.defineProperty(self.honeyRequire, 'resolve', {'value':resolve});
        Object.defineProperty(self.honeyRequire, 'path', {'get':function(){return requirePath.slice(0);}});
    }
    catch (e) {
        // NOTE IE8 can't use defineProperty on non-DOM objects, so we have to fall
        //      back to unsave property assignments in this case.
        self.honeyRequire = honeyRequire;
        self.honeyRequire.resolve = resolve;
        self.honeyRequire.path = requirePath.slice(0);
    }
...
//      in strict mode, too.
    ...
    function /*load*/(module/*, cache, pwd, source*/) {
        ...
        if (typeof module.id !== 'string')
            for (id in module)
                arguments[1]['$'+honeyRequire.resolve(id).id] = module[id].toString();
        arguments[2].shift();
    }
